<div>
        ...
        <x-table>
            ...
            <x-slot name="content">
                @forelse ($collection as $index => $item)
                    <x-tr>
                        <x-td wire:key="ite-{{ $item->id }}">{{ $item->no_empleado }}</x-td>
                        <x-td>{{ $item->nombre . ' ' . $item->ap_paterno . ' ' . $item->ap_materno}}</x-td>
                        <x-td>{{ $item->usuario->email }}</x-td>
    ...
    <x-dialog-modal wire:model="modal">
        <x-slot name="title">
            <h2 class="text-center">Metodo</h2>
        </x-slot>
        <x-slot name="content">
