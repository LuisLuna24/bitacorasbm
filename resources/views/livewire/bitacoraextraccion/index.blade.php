<div wire:poll>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400"
            ...
            <tbody>
                @foreach ($extraccions as $extraccion)
                    <tr
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th scope="row"
                            class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center"
                            wire:key="analisis-{{ $extraccion->id }}">
                            {{-- Mostrar no_registro de extraccion --}}
                            @foreach ($extraccion->extraccions as $pc)
                        ...
                        <td class="px-6 py-4 text-center">
                            {{ $extraccion->reactivo->nombre }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            {{ $extraccion->fecha_apertura }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            {{ $extraccion->reactivo->fecha_caducidad }}
                        </td>
                        <td class="px-6 py-4 text-center">
                        ...
                        @if (auth()->user()->nivel != 3)
                            @if ($extraccion->validacion == 'Sin Validacion')
                                <td class="px-6 py-4 text-center">
                                    <x-button wire:click="edit({{ $extraccion->id }})"><svg
                        ...
                        <td class="px-6 py-4 text-center">
                            <x-button wire:click="view({{ $extraccion->id }})"><svg xmlns="http://www.w3.org/2000/svg"
    ...
    <x-dialog-modal wire:model="create_new">
        ...
        <x-slot name='content'>
            @livewire('bitacoraextraccion.reactivoextraccions-new')
        </x-slot>
        <x-slot name='footer'>
            ...
            <form class="grid gap-3" wire:submit="update">
                <div class="grid grid-cols-1 max-md:grid-cols-1 gap-5">
                    ...
                    <div class="grid gap-3 w-full">
                        <div class="w-full">
                            <x-input wire:model.live="search_registro" placeholder="Buscar(no_registro)"
                                class="w-full" />
                        </div>
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @foreach ($rextraccions as $rextraccion)
                                        <tr
                                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <td scope="row"
                                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center"
                                                wire:key="rpcr-{{ $rextraccion->id }}">
                                                <x-checkbox wire:model="rpcrEdit.selectedTagsPcr"
                                                    value="{{ $rextraccion->id }} " /><span
                                                    class="text-white">{{ $rextraccion->no_registro }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    <div class="grid gap-3 w-full">
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @foreach ($vrextraccions as $vrextraccion)
                                        <tr
                                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <td scope="row"
                                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center"
                                                wire:key="rextraccion-{{ $vrextraccion->id }}">
                                                {{ $vrextraccion->extraccion->no_registro }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <x-dialog-modal wire:model="version_register">
        ...
        <x-slot name='content'>
            @livewire('bitacoraextraccion.versionesreactivoextraccions', ['VercionReactivoId' => $VercionReactivoId])
        </x-slot>
        <x-slot name='footer'></x-slot>
    </x-dialog-modal>
