<div>
    ...
    <div class="flex gap-3 m-2 max-md:flex-col">
        ...
        <x-input class="w-full" placeholder="Buscar Metodo (nombre)" wire:model.live="search" />
            ...
            <tbody>
                @foreach ($metodos as $metodo)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center" wire:key="analisis-{{ $metodo->id }}">
                                {{$metodo->nombre}}
                            </th>
    ...
    <x-dialog-modal wire:model="version_view">
        <x-slot name='title'>
            <h2 class="text-center">Verciones Metodo</h2>
        </x-slot>
        <x-slot name='content'>
            @livewire('metodos.version',[
                'VersionMetodoId'=>$VersionMetodoId
            ])
        </x-slot>
        <x-slot name='footer'></x-slot>
    </x-dialog-modal>
    ...
    <x-dialog-modal wire:model="update_new">
        ...
        <x-slot name='content'>
            <form wire:submit="update">
                <div>
                    <x-label>Nombre:</x-label>
                    <x-input wire:model="metodoEdit.nombre" type="text" class="block mt-1 w-full" />
                    <x-input-error for="metodoEdit.nombre" />
                </div>
                <div class="mt-5 flex justify-around">
